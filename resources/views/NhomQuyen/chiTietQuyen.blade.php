        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <h4 class="card-title">Nhóm quyền: {{$nhomQuyen->permissionGroup_name}}</h4>
                        <br>
                        <h5>Quyền cơ bản</h5>
                        <br>
                                        ...
                                        @php $trangThai =0; @endphp
                                            @foreach($quyenCoBan as $item2)
                                                ...
                                                && $item2->chiTietQuyen_id == $item1->chiTietQuyen_id)
                                                     @php $trangThai =$item2->permissionDetail_status; @endphp
                                                @endif
                                            @endforeach
...
<script>
    function capNhatTrangThai(idQuyen,idChiTiet)
    {
        ...
        $idNhomQuyen = {{$nhomQuyen->permissionGroup_id}};
            $.ajax({
            type: 'get',
            url: '{{ route("capNhatTrangThaiNhomQuyen")}}',
            data: {
                'idQuyen': idQuyen,
                'idChiTiet': idChiTiet,
                'trangThai':$trangThai,
