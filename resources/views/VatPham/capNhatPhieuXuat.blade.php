        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    {{ csrf_field() }}
                        <div class="row">
                            <input hidden id="id" name="id" value="{{$phieuXuat->deliveryBillItem_id}}">
                            
                            <div class="col-lg-6 col-sm-6">
                                <label>Nội dung <span style="color: red">*</span></label>
                                <input maxlength="30" required class="form-control" name="noiDung" value="{{$phieuXuat->deliveryBillItem_name}}">
                            </div>
                            <div class="col-lg-6 col-sm-6">
                            ...
                            <div class="col-lg-12 col-sm-6">
                                <br>
                                @if($phieuXuat->deliveryBillItem_status==1)
                            ...
                            <div id="ngoaiBo" style="display: none">
                                ...
                                <div class="row">
                                    <div class="col-lg-6 col-sm-6">
                                        ...
                                        <input maxlength="30" required class="form-control"
                                         name="nguoiNhan" value="{{$phieuXuat->deliveryBillItem_receiver}}">
                                    </div>
                                    <div class="col-lg-6 col-sm-6">
                                        <label>Bộ phận <span style="color: red">*</span></label>
                                        <input maxlength="30" required class="form-control" 
                                        name="boPhan" value="{{$phieuXuat->deliveryBillItem_partName}}">
                                    </div>
                                </div>
                            </div>
                            @if($phieuXuat->deliveryBillItem_status==1)
                                ...
                                <div class="row">
                                    <div class="col-lg-6 col-sm-6">
                                       ...
                                       <select class="js-example-responsive"  style="width:100%" name="nguoiNhan2" >
                                        ...
                                        @if($item->employee_name== $phieuXuat->deliveryBillItem_receiver)
                                    ...
                                    <div class="col-lg-6 col-sm-6">
                                        ...
                                        <select class="js-example-responsive"  style="width:100%" name="boPhan2" >
                                            ...
                                            @if($item->department_name== $phieuXuat->deliveryBillItem_partName)
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-12 col-sm-6">
                                <label>Ghi chú</label>
                                <textarea maxlength="200"  class="form-control" name="ghiChu">{{$phieuXuat->deliveryBillItem_note}}</textarea>
                            </div>
                            <div class="col-lg-4 col-sm-6">
                                <label>Vật phẩm</label>
                                <select class="js-example-responsive" style="width:100%" id="sanPham" name="sanPham">
                                    @foreach($sanPham as $item)
                                    @php $soLuong = $item->inventoryItem_amount; @endphp
                                    @foreach($chiTiet as $item1)
                                       @if ($item->item_id==$item1->item_id)
                                       @php $soLuong += $item1->deliveryBillItemDetail_amount; @endphp
                                       @endif
                                    @endforeach
                                    <option value="{{$item->item_id}}">{{$item->item_name}} ({{ $soLuong }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-4 col-sm-6">
                            ...
                            <table class="table  table-bordered " id="danhSachSanPham">
                                ...
                                <tbody>
                                    ...
                                    <td id="soLuong1{{$item->item_id}}">
                                        <input class='myInput'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='change({{$item->item_id}});' style='width: 50%; text-align: right; '  id='soLuong{{$item->item_id}}' name='soLuong{{$item->item_id}}' value='{{$item->deliveryBillItemDetail_amount}}'>
        ...
        @foreach($sanPham as $item)   
            @php $soLuong = $item->inventoryItem_amount; @endphp
             @foreach($chiTiet as $item1)
                ...
                @endif
             @endforeach
        <input hidden id="tonKho{{$item->item_id}}" value="@php echo $soLuong; @endphp">
        @endforeach
...
function btnThem()
    {
     ...
     function themSanPham(id,soLuong1) {
            ...
            if(soLuong>0)
                {
                    @foreach($sanPham as $item)
                        ...
                        if ($('#soLuong{{$item->item_id}}').val()>=0 && sanPham == {{$item->item_id }})
                        {
                           ...
                           if ($soLuongThem <= $('#tonKho{{$item->item_id}}').val())
                           {
                        ...
                        else
                        {
                            if (sanPham == {{$item->item_id}}) {
                                if (parseInt(soLuong)  <=parseInt( $('#tonKho{{$item->item_id}}').val())) {
                                    ...
                                    cell1.innerHTML = ten;
                                    cell3.innerHTML = "<input class='myInput'  onkeypress='return event.charCode >= 48 && event.charCode <= 57' onkeyup='change({{$item->item_id}});' style='width: 50%; text-align: right; '  id='soLuong" + sanPham + "' name='soLuong" + sanPham + "' value='" + soLuong + "'>" +
                                        "<button class='btn mb-1 btn-outline-primary'  type='button' onclick='themSanPham({{$item->item_id}},1);' ><i class='fa fa-plus'></i></button>" +
                                        "<button class='btn mb-1 btn-outline-danger' type='button' onclick='truSanPham({{$item->item_id}},1);'><i class='fa fa-minus'></i></button>";
      ...
      $('#myform1').submit(function() {
        $.ajax({
            type: 'Post',
            url: '{{ route("postCapNhatPhieuXuatVatPham")}}',
            ...
            success: function(data) {
                if (data == 1) {
                    ...
                    setTimeout(function() {
                        window.location = "{{route('getXuatVatPham')}}";
                    }, 2000);
