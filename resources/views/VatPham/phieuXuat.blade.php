                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-3 col-sm-6">
                            @if(session('quyen252')==1)
                            <a href="{{route('getThemPhieuXuatVatPham')}}"><button type="button" class="btn mb-1 btn-outline-success" 
                        ...
                        <table class="table table-striped table-bordered ">
                            <thead>
                                <tr>
                                    ...
                                    <th>Người nhận</th>
                                    <th>Chi tiết</th>
                                    @if(session('quyen253')==1)
                                    <th>Sửa</th>
                                    @endif
                                    @if(session('quyen254')==1)
                                    <th>Xóa</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody id="duLieuSearch">
                                ...
                                <tr>
                                    ...
                                    @if(session('quyen253')==1)
                                    <td><a class="btn" href="{{route('getCapNhatPhieuXuatVatPham')}}?id={{$item->deliveryBillItem_id}}" >
...
<script>
    function getChiTiet(id)
    {
        $.ajax({
            type: 'get',
            url: '{{route("getChiTietPhieuXuatVatPham")}}',
            data: {
                'id': id
            },
...
$.ajax({
    type: 'get',
    url: '{{ route("searchPhieuXuatVatPham")}}',
    data: {
        'value': $value,
        'page': 1
    },
        ...
        swal({
            ...
            function(e) {
                e ? swal(
                        $.ajax({
                            type: 'get',
                            url: '{{route("getXoaPhieuXuatVatPham")}}',
                            data: {
                                'id': id
                            },
                            success: function(data) {
