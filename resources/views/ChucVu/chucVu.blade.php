                        ...
                        <div class="row">
                            <div class="col-lg-3 col-sm-6">
                                <div class="input-group icons">
                                    ...
                                    placeholder="Search tên chức vụ" aria-label="Tìm marketing">
                                </div>
                            </div>
                        ...
                        <table class="table table-striped table-bordered ">
                            ...
                            <tbody id="duLieuSearch">
                                ...
                                <tr>
                                    ...
                                    @if(session('quyen353')==1)
                                    <td><a class="btn" data-toggle="modal" data-target="#basicModal2"  onclick="setValue('{{$item->position_id}}','{{$item->position_name}}');">
...
<script>
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data == 1) {
                                    ...
                                    window.location = "{{route('getChucVu')}}";
                                } else if (data == 2) {
                                    KiemTra("Xóa chức vụ", "Bạn không có quyền xóa!!!!");
                                } else
                                    PhatHienLoi("Xóa chức vụ", "Lỗi Kết kết nối!!!!");
                            }
                        })
                    ) :
    ...
    $('#myform1').submit(function() {
        $.ajax({
            ...
            success: function(data) {
                if (data == 1) {
                    ThemThanhCong("Thêm chức vụ", "Thêm thành công!!!");
                    setTimeout(function() {
                        window.location = "{{route('getChucVu')}}";
                    }, 2000);

                } else if (data == 2) {
                    KiemTra("Thêm chức vụ", "Bạn không có quyền thêm!!!");
                }

                else if (data == 3) {
                    KiemTra("Thêm chức vụ", "Mã chức vụ đã tồn tại!!!");
                }                 else {
                    PhatHienLoi('Thêm chức vụ', "Lỗi Kết Nối!!!");
                }
    ...
    $('#myform2').submit(function() {
        $.ajax({
            ...
            success: function(data) {
                if (data == 1) {
                    CapNhatThanhCong("Cập nhật chức vụ", "Cập nhật thành công!!!");
                    setTimeout(function() {
                        window.location = "{{route('getChucVu')}}";
                    }, 2000);

                } else if (data == 2) {
                    KiemTra("Cập nhật chức vụ", "Bạn không có quyền cập nhật!!!");
                } 
                else if (data == 3) {
                    KiemTra("Cập nhật chức vụ", "Mã chức vụ đã tồn tại!!!");
                } 
                else {
                    PhatHienLoi('Thêm chức vụ', "Lỗi Kết Nối!!!");
                }
