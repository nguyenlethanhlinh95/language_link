            ...
            <div class="modal-content">
                <form id="myform1" autocomplete="off" action="" enctype="multipart/form-data" method="post">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title">Thêm Teamwork</h5>
                        <button type="button" class="close" data-dismiss="modal"><span>×</span>
                        </button>
                    </div>
                    <div class="modal-body">
                      
                        <label>Tên Teamwork <span style="color: red">*</span></label>
            ...
            <div class="modal-content">
                <form id="myform2" autocomplete="off" action="" enctype="multipart/form-data" method="post">
                    ...
                    <div class="modal-body">
                        ...
                        <label>Chi nhánh <span style="color: red">*</span></label>
                        <select class="form-control" id="chiNhanh2" name="chiNhanh2" required>
                            @foreach($chiNhanh as $item)
                            <option value="{{ $item->branch_id }}">{{ $item->branch_name }}</option>
                            @endforeach
                        </select>
                        ...
                        <table class="table table-striped table-bordered ">
                            ...
                            <tbody id="duLieuSearch">
                                ...
                                <tr>
                                    ...
                                    @if(session('quyen371')==1)
                                    <td><a class="btn" href="{{ route('getNhanVienTeam') }}?id={{ $item->team_id }}"><i class="fa fa-list"></i></a></td>
                                    @endif
                                    @if(session('quyen363')==1)
...
<script>
    function setValue(id,ten,chiNhanh)
    {
        $('#id2').val(id);
        $('#ten2').val(ten);
    ...
    $('#myform2').submit(function() {
        $.ajax({
            ...
            success: function(data) {
                if (data == 1) {
                    ThemThanhCong("Cập nhật teamwork", "Cập nhật thành công!!!");
                    setTimeout(function() {
                        window.location = "{{route('getTeam')}}";
                    }, 2000);
                } else if (data == 2) {
                    KiemTra("Cập nhật teamwork", "Bạn không có quyền cập nhật!!!");
                }               
                else {
                    PhatHienLoi('Cập nhật teamwork', "Lỗi Kết Nối!!!");
                }
