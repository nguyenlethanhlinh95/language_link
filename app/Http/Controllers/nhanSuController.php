...
class nhanSuController extends Controller
{
     ...
     */
    public function __construct(EmployeeService $employeeService, BranchService $branchService)
    {
    ...
    public function getNhanSu()
    {
        try {
            ...
            if($quyenChiTiet==1)
            {
                ...
                if (isAdmin()) {
                    $nhanSu = $this->employeeService->getAllEmployee($pagination);
                } else {
                    $nhanSu = $this->employeeService->getAllEmployeeFromBranch($user['branch_id'], $pagination);
                }
                $chiNhanh = $this->branchService->getAllBranch();
                $soKM = $this->employeeService->getTotalEmployee();
                $soTrang = (int) $soKM / $pagination;
                if ($soKM % $pagination > 0)
                    $soTrang++;
                $data = [
                    'chiNhanh' => $chiNhanh,
                    'nhanSu'   => $nhanSu,
                ...
                return view('NhanSu.nhanSu', $data);
            }
            else
            {
    ...
    public function getThemNhanSu()
    {
        ...
        if($quyenChiTiet==1)
        {
            ...
            return view('NhanSu.themNhanSu',$data);
        }
        else
        {
            return redirect()->back();
...
//            $chiNhanh = $request->get('chiNhanh');
            if (isAdmin()) {
                ...
                $chiNhanh = $user['branch_id'];
            }
            $page = $request->get('page');
            if ($value == "")
            {
                if($chiNhanh == config('constant.SEARCH_BRANCH.ALL'))
                {
